...
if __name__ == '__main__':
    ...
    X = images
    Y = voting_model.predict(X, batch_size=15, verbose=1)

    def process_pred(input):
        preds_max = np.max(input, axis=1, keepdims=True)
        y = np.equal(input, preds_max).astype(np.float32)
